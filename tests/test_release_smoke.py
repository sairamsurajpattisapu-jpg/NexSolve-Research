...
def test_release_smoke_demo_scenarios_listing():
    response = client.get("/api/demo/scenarios")
    assert response.status_code == 200
    scenarios = response.json()
    assert len(scenarios) == 7
    ids = [s["id"] for s in scenarios]
    assert "NORMAL_TRAFFIC" in ids
    assert "EARLY_ATTACK_SIGNAL" in ids
    assert "SUSTAINED_ATTACK_FORECAST" in ids
    assert "CONTRADICTORY_EVIDENCE" in ids
    assert "UNKNOWN_BEHAVIOR" in ids
    assert "FORECAST_ABSTAINED" in ids
    assert "POOR_CAPTURE_QUALITY" in ids


def test_release_smoke_each_demo_scenario_and_reports():
    scenario_ids = [
        "NORMAL_TRAFFIC",
        "EARLY_ATTACK_SIGNAL",
    ...
    for sid in scenario_ids:
        # 1. Scenario Endpoint
        res = client.get(f"/api/demo/scenarios/{sid}")
        assert res.status_code == 200, f"Failed for scenario {sid}"
        payload = res.json()
        assert payload["is_demo"] is True
        assert payload["demo_scenario_id"] == sid
        assert "processing_metrics" in payload

        # Check Attack Horizon
        horizon_obj = payload.get("attack_horizon")
        assert horizon_obj is not None
        evidence_chain = horizon_obj.get("evidence_chain", [])
        assert len(evidence_chain) == 5
        # Ensure ordering T+1 to T+5
        assert [h["horizon"] for h in evidence_chain] == [1, 2, 3, 4, 5]
        # Calibration is unsupported / uncalibrated
        assert horizon_obj.get("confidence_summary", {}).get("calibration_status") == "UNSUPPORTED"

        if sid in ("FORECAST_ABSTAINED", "POOR_CAPTURE_QUALITY"):
            # Abstention case
            assert payload.get("abstention") is not None
            assert payload["abstention"].get("abstained") is True or payload["abstention"].get("status") in ("ABSTAINED", "FORECAST_ABSTAINED")
            assert "reason" in payload["abstention"]
        ...
        # Check Quality
        quality = payload.get("quality") or payload.get("capture_quality")
        if quality:
            if sid == "POOR_CAPTURE_QUALITY":
                assert quality["status"] == "DEGRADED"

        # 2. JSON Report
        json_res = client.get(f"/api/demo/scenarios/{sid}/report.json")
        assert json_res.status_code == 200
        report_data = json_res.json()
        assert "report_id" in report_data
        assert "provenance" in report_data.get("sections", {})
        assert "capture_hash" in report_data.get("sections", {}).get("provenance", {})

        # 3. HTML Report
        html_res = client.get(f"/api/demo/scenarios/{sid}/report.html")
        assert html_res.status_code == 200
        html_text = html_res.text
        assert "<!DOCTYPE html>" in html_text
        assert "NexSolve Report" in html_text
        # Air-gapped: no external script tags or external CDNs
        assert "https://cdn." not in html_text
        assert "<script src=\"http" not in html_text


# ==============================================================================
# 2. DEMO DETERMINISM TEST
# ==============================================================================

def test_demo_scenarios_deterministic_multiple_runs():
    for sid in ["NORMAL_TRAFFIC", "SUSTAINED_ATTACK_FORECAST", "FORECAST_ABSTAINED"]:
        res1 = client.get(f"/api/demo/scenarios/{sid}").json()
        res2 = client.get(f"/api/demo/scenarios/{sid}").json()

        # Attack Horizon determinism
        assert res1.get("attack_horizon") == res2.get("attack_horizon")
        # Evidence chain determinism
        assert res1.get("evidence_chain") == res2.get("evidence_chain")
        # Abstention state determinism
        assert res1.get("abstention") == res2.get("abstention")
        # Metric counts
        assert res1.get("packet_count") == res2.get("packet_count")
