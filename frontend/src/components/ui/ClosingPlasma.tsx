...
export function ClosingPlasma({
  ...
  const resolvedIsDark =
    themeMode === 'light' ? 0 : themeMode === 'dark' ? 1 : isDark ? 1 : 0
  const isDarkRef = useRef(resolvedIsDark)

  useEffect(() => {
    isDarkRef.current = resolvedIsDark
  }, [resolvedIsDark])
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      if (!settings.interactive || isReducedMotion) {
        return
      }
    ...
    // Attach to window so pointer movement is captured across entire screen without blocking clicks
    ...
    const render = (now: number) => {
      const elapsed = isReducedMotion ? 0 : (now - start) / 1000
      ...
      gl.uniform1f(uSpeed, isReducedMotion ? 0 : settings.speed)
      gl.uniform1f(uTurbulence, settings.turbulence)
      gl.uniform1f(uMouseInfluence, isReducedMotion ? 0 : settings.mouseInfluence)
      gl.uniform1f(uGrain, settings.grain)
      gl.uniform1f(uSparkle, isReducedMotion ? 0 : settings.sparkle)
  ...
  return (
    <div
      ref={containerRef}
      className={`closing-plasma-root ${className}`.trim()}
      style={{
        position: 'relative',
        width: '100%',
        height: '100%',
