...
export function Reports() {
  ...
  const findings = report.detection.findings ?? []
  ...
  return <div className="page-stack page-enter">
    ...
    <Panel><SectionHeading title="Finding register" description="The most recent evidence-based events in this analysis." />{findings.length === 0 ? <p className="body-copy">No detections were returned for this analysis.</p> : <div className="report-table"><div className="table-row table-head"><span>Finding</span><span>Category</span><span>Severity</span><span>Risk</span></div>{findings.slice(0, 12).map((finding) => <div className="table-row" key={finding.finding_id}><span>{finding.finding_id}</span><span>{finding.attack_category.replaceAll('_', ' ')}</span><span><SeverityPill severity={finding.severity} /></span><strong>{Number.isFinite(finding.risk_score) ? finding.risk_score.toFixed(1) : 'Unavailable'}</strong></div>)}</div>}</Panel>
  </div>
}
